<template>
  <v-container class="d-flex flex-row flex-wrap" width="100%">
    <v-card
      ...
      class="ma-5"
    >
      <v-card-title> Step {{ i + 1 }}: {{ step.name }} </v-card-title>
      <overlay-image :src="step.src ?? 'https://placekitten.com/g/601/300'" />
      ...
      <v-card-actions v-if="step.showUploader">
        <UploadFoundry />
      </v-card-actions>
    </v-card>
  </v-container>
...
export default {
  ...
  data: () => ({
    ...
    steps: [
      {
        name: "Welcome!",
        description: "Hey there! Lets get FoundryVTT installed! :)",
      },
      {
        ...
        src: "../assets/foundryvtt-click-username.png",
      },
      {
        ...
        src: "../assets/foundryvtt-click-timed-url.png",
      },
      {
