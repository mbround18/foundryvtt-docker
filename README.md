...
```sh
docker run --rm -p 4444:4444 -e HOSTNAME="127.0.0.1" -e SSL_PROXY="false" -v ${PWD}/data:/foundrydata mbround18/foundryvtt-docker:latest
```
...
5. Configure the Ingress for the web client
   Note: As a reminder, I am using https://k3s.io for my deployment. So when it comes to ingress, it ships with traefik and you might have to alter the configuration to match your setup. 
   > ${PWD}/04-ingress.yml
   ```yaml
   ---
